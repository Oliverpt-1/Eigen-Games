...
async function sendTask(proofOfTask, data, taskDefinitionId) {
  ...
  // Convert data to JSON string before encoding
  const jsonString = JSON.stringify(data);
  
  data = ethers.hexlify(ethers.toUtf8Bytes(jsonString));
...
async function getIPfsTask(cid) {
    ...
    // Return the full data instead of just extracting symbol and price
    return data;
}
